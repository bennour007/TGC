...
We will use data from **Malawi HIV Program** during the four quarters of 2019, you can access the data yourself [here](https://dms.hiv.health.gov.mw/dataset/?tags=HIV&res_format=XLSX&year=2019).
...
files <- list.files(path=here::here("data", "raw"), full.names = T)

data <- map(files, function(x) readxl::read_xlsx(x))
...
We can do this for the `new_negative` column for example, we can use a different kind of palette, I'm using for this case the green palette from the same package: [`ggsci::green_material`](https://github.com/nanxstats/ggsci) , you can find all the palettes included in the `paletteer` package in [here](https://emilhvitfeldt.github.io/paletteer/#included-packages).
...
Since 4 of our columns are conveniently named: [previous/new \_ negative/poitive ] *.* We can use this to our advantage to easily create spanners using the delimitation '\_*'* to differentiate between the spanner label and the column label by means of the *`tab`*`_spanner_delim` function:
